...
// Interface ini mendefinisikan properti yang dibutuhkan oleh komponen FeaturedDestinations, termasuk kategori yang dipilih, ID untuk keperluan scrolling, dan fungsi dari parent untuk menangani perubahan kategori.
...
// Komponen FeaturedDestinations menerima props kategori yang dipilih, ID untuk scrolling, dan fungsi untuk mengubah kategori; komponen ini juga mengatur state untuk mengelola efek hover dan gambar aktif pada destinasi yang ditampilkan.
export const FeaturedDestinations = ({ selectedCategory, id, onSelectCategory }: FeaturedDestinationsProps) => {
  const [isHovered, setIsHovered] = useState<number | null>(null); // Menyimpan indeks item yang sedang di-hover
  const [activeImage, setActiveImage] = useState<{ [key: number]: number }>({}); // Menyimpan indeks gambar aktif untuk setiap item destinasi
...
// id diisi dengan id dari section ini,id berada di index.tsx
  return (
    <section id={id} className="py-16 bg-gradient-to-b from-white to-travel-50"> 
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 flex justify-center">
          {/* ... Tombol Discover More Destinations  */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
